...
for k = 1:V
    IND = find(CB(:,:,k)); % gives no. of non-zero element
    [I, ~] = ind2sub(s, IND); % no. to index conversion
    F(unique(I),k) = 1; % finds unique RE used by a user
end
...
% Step 2: Iterative procedure
    for iter = 1:Nit
        % Igv update FN ------> VN (Complex)
        for k = 1:K
            ind = find(F(k,:)==1); % finds the users active on a RE
